...
size = 2000
list_id = [i for i in range(x_test.shape[0]) if y_test[i]<=150000]
x, y = x_test[list_id][:size], y_test[list_id][:size]
x_rules, y_rules = x_train[:10], y_train[:10]

results = RunExperiments(ac_explainer, x_train, x_test, y_train, y_test, columns_name)
down = np.array(x.shape[0]*[250000.])
up = np.array(x.shape[0]*[400000.])
results.ddp_importance_local, results.ddp_index_local, results.size_local, results.ddp_local = results.acv_explainer.importance_cdp_rf(x, y, down, up, x_train, y_train, t=0,
                                                 stop=False, pi_level=0.2)

results.S_star_local, results.S_bar_set_local = acv_explainers.utils.get_active_null_coalition_list(results.ddp_index_local, results.size_local)

results.ddp_local, results.w_local = results.acv_explainer.compute_cdp_weights(x, y, down, up, x_train, y_train,
                                                                              S=results.S_bar_set_local)
...
results.counterfactuals_samples_w_local = return_global_counterfactuals_reg(results.acv_explainer, x, y, down, up,
                                                                     results.S_star_local, results.S_bar_set_local,
...
run_sampling_local_counterfactuals(results, x, y, down, up)
